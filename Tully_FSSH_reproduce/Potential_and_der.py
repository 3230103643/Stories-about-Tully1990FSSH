...
def SAC_potential(position, dx = 0.001): # 计算Simple avoided crossing case 的哈密顿矩阵及其导数
    V = SAC_V(position)
    V_der = (SAC_V(position+dx)-V) / dx
    return V, V_der
...
def DAC_potential(position, dx = 0.001): # 计算Dual avoided crossing case 的哈密顿矩阵及其导数
    V = DAC_V(position)
    V_der = (DAC_V(position+dx) - V) / dx
    return V, V_der
...
def ECR_potential(position, dx=0.001): # 计算Extended Coupled avoided crossing case 的哈密顿矩阵及其导数
    V = ECR_V(position)
    V_der = (ECR_V(position+dx) - V) / dx
    return V, V_der
